...
I have been blogging since 2017 when I was young. Over years, I have used many blogging systems, some dynamic ones mostly written in PHP, such as WordPress and Typecho, and some static ones like Hexo and Hugo. The two approaches have their own pros and cons. Ultimately, I transitioned from dynamic blogging platforms to a _static_ website (well, it's not static build, we require server-side rendering).
...
The whole blog is developed with Git, so it's easy to have of idea to manage of posts in Git, that is my current way to keep to things tidy. I have different Git branches for blog feature development and writing blogs, finally they will be merged into main branch and ship and deploy to Vercel.
...
-   The `main` branch for live blog site currently deployed to Vercel.
-   The `blog` branch is where I draft new posts and write content in MDX.
-   The `dev` branch contains features and bugfix for the blog system, isolation between `blog` and `dev` allows me to test changes without impacting live branch or in-progress writing.
...
Grouping all images for a post is efficient and orderly. Nextra's [Static Next Image](https://nextra.site/docs/guide/image) lets me consolidate media in one directory along with post content:

```bash
.
├── antfu.me_posts.png
├── index.mdx                # post MDX file
├── mindmap.png
└── react-vs-vue.png
```

Nextra uses a MDX plugin to parse all images into statically imported images enveloped by `next/image`. But there are some issues for the image handling by Nextra, it's not easy to customize an MDX image component, since it will disrupt of the static-import workflow. Fortunately, I can patch the package to work around this issue (see [this commit](https://github.com/jukrb0x/wh0.is/commit/fdeb9fe55c22e0fe774768f4bdfff61136156dc6)) for now. Additionally, The relative URLs are not handled for the Open Graph Image (og:image) in meta. I'm still working to reconcile these issues.
